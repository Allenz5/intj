...
passage, say what you want done, and intj spins up a coding agent (Claude Code by default) in
...
1. **Pick a project folder** — any git repo (a non-repo is `git init`ed for you). On this start
   ...
   Both are saved per project under `.intj/settings.json` and reloaded into the inputs when you
   browse back to that folder.

2. **Pick or create a workload** — a workload is a set of markdown docs kept under
   `.intj/<timestamp>-<uuid>/`, starting from `main.md`. Docs live outside git and are never
   committed.
...
- `skills/` — the agent skills intj invokes (`update-docs`, `split-doc`, `merge-doc`, …).
- `.intj/` — per-project workloads, docs, session worktrees, and `settings.json`. Kept out of git.
