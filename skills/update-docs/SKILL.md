---
name: update-docs
description: Update the project's markdown docs so they describe the current state of the work. Use when the user says "/intj:update-docs", asks to update the README or docs, or when a piece of work is finished and the docs that govern it are now out of date.
---
...
2. **Find the docs that govern it.** Start from the doc this session was opened from, if the user named one. Otherwise walk the doc tree from `README.md` down and pick the docs whose scope covers the changed code. A change can touch several docs; a doc's parent needs editing only if the change alters what the parent says about it.
