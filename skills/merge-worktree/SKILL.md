---
name: merge-worktree
description: Merge one or more intj worktree branches into the current branch, then remove the merged worktrees and branches. Use when the user says "/intj:merge-worktree", asks to merge worktrees or sessions, or wants to fold finished sessions back in.
---

# Merge worktree

In intj, one AI session is one git worktree on a branch `intj/<id>` under `.intj/worktrees/<id>`. This skill merges any number of them into the branch checked out where the skill runs.
...
2. **Check that nothing will be lost.**
   ...
   - For each source worktree, run `git -C <path> status --porcelain`. If it has uncommitted changes, stop and ask the user whether to commit them there first, or skip that worktree.
...
4. **Clean up merged worktrees.** For each branch that merged, check `git merge-base --is-ancestor intj/<id> HEAD`, then:

   ```sh
   git worktree remove <path>
   git branch -d intj/<id>
   ```

   Use `-d`, never `-D`. Leave any worktree that did not merge untouched.
